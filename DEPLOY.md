...
### Q. 내가 Streamlit을 실행해야 사람들이 볼 수 있어?

**아니요.**  
배포한 앱은 **Streamlit Cloud 서버**에서 24시간 돌아갑니다.  
- 본인 PC를 끄거나, `streamlit run`을 하지 않아도 **배포된 URL만 있으면 누구나 접속**해서 쓸 수 있습니다.  
- 링크를 공유한 사람은 **언제든** 브라우저로 들어가서 사용하면 됩니다.
...
1. **앱이 계속 떠 있는지**  
   → 예. 한 번 Deploy 되면 **계속 실행**됩니다. (무료 플랜에서도 일정 시간 사용 없으면 sleep 했다가 접속 시 깨우는 경우는 있을 수 있음.)

2. **코드 수정 시 반영**  
   → GitHub `main` 브랜치에 **push**하면 Streamlit Cloud가 **자동으로 다시 빌드·배포**합니다.  
   별도로 “다시 Deploy” 버튼 누를 필요 없이, `git push`만 하면 됩니다.
...
2. 내 앱 목록에서 해당 앱 클릭.
3. 여기서 할 수 있는 것:
   - **URL 확인/복사**: 다른 사람에게 링크 공유.
   - **Logs**: 에러·출력 로그 확인.
   - **Reboot app**: 수동으로 앱 재시작 (이상 동작 시).
   - **Settings**: 저장소, Branch, Main file path 변경.
   - **Delete app**: 배포 삭제.

### Q. URL을 아무한테나 공유해도 돼?

- **Public** 저장소로 배포했다면, **URL만 알면 누구나** 접속 가능합니다.
- 비공개로 쓰고 싶다면: Streamlit Cloud 무료 플랜은 보통 공개 앱만 지원하므로, 비공개가 필요하면 유료 옵션이나 다른 호스팅을 찾아봐야 합니다.
